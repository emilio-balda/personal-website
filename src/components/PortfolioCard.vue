...
<template>
  <div
    class="neumorphic-shadow-2 h-[35rem] max-w-[20rem] min-w-[15rem] flex flex-col items-center justify-center overflow-hidden rounded-lg text-xl md:h-[35rem] md:w-72 dark:bg-background-900 md:transition-all md:duration-300 md:hover:-translate-y-2"
  >
    <div
    ...
    <div class="h-7/12 w-3/4 flex flex-col items-center justify-between">
      <div>
        <div v-if="props.aliasToTag" class="hidden gap-2 py-2 md:flex md:flex-wrap md:items-center md:justify-center">
          <p v-for="alias in props.item?.tag_aliases" :key="alias" class="whitespace-nowrap text-sm text-primary-700">
      ...
      <div class="pb-4">
        <glowing-button type="ghost" @click="knowMore(props.item?.id)">
          Know More
        </glowing-button>
      </div>
    </div>
